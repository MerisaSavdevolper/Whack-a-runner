...
function getTimeDelay(difficulty) {
    if (difficulty === "easy") {
        ...
        throw new Error("Invalid difficulty level"); console.log(getTimeDelay("easy")); // Expected output: 1500
console.log(getTimeDelay("normal")); // Expected output: 1000
console.log(getTimeDelay("hard")); // Expected output: Random between 600 and 1200
...
function chooseHole(holes) {
    ...
    // Check if the chosen hole is the same as the last hole
    if (hole === lastHole) {function chooseHole(holes) {
    let index;
    let hole;
    
    // Keep choosing a random hole until it's different from lastHole
    do {
        index = Math.floor(Math.random() * holes.length); // Random index
        hole = holes[index];
    } while (hole === lastHole);
    
    lastHole = hole; // Update the lastHole
    return hole; // Return the chosen hole
}
        // If it is, call chooseHole again to get a different hole
  ...
  function chooseHole(holes) {
    ...
    do {
        index = Math.floor(Math.random() * holes.length); // Choose random index
        hole = holes[index]; // Get the hole from the list
    } while (hole === lastHole); // Continue while it's the same as lastHole
    
    lastHole = hole; // Update the last chosen hole
    return hole; // Return the new hole
}
    
    // If the hole is different from the last one, update lastHole
    lastHole = hole;

    return hole; // Return the chosen hole
}let lastHole = null; // Initialize lastHole variable to track the previous selection

function chooseHole(holes) {
    // Ensure there's at least one hole in the list
    if (holes.length === 0) {
        throw new Error("The list of holes is empty");
    }

    // Generate a random index and select a random hole
    const index = Math.floor(Math.random() * holes.length);
    const hole = holes[index];

    // If the hole is different from the last one, update lastHole and return it
    if (hole !== lastHole) {
        lastHole = hole; // Update lastHole
        return hole; // Return the selected hole
    } else {
        // Otherwise, select a different hole
        return chooseHole(holes); // Recursive call to ensure a different hole
    }
}
...
console.log(randomHole); // Should return a random hole that isn't the last one
